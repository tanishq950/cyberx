...
class Totals extends Base {
    ...
    public function process(): void {
        ...
        $start = time();
        $models = \CyberX\Utils\Constants::get()->REST_TOTALS_MODELS;
        ...
        foreach ($models as $name => $modelClass) {
            ...
            foreach ($keys as $key) {
                (new \CyberX\Models\SessionStat())->updateStats($key);

                $cnt = $model->updateAllTotals($key);
                $res[$name]['cnt'] += $cnt;
                if (time() - $start > \CyberX\Utils\Constants::get()->ACCOUNT_OPERATION_QUEUE_EXECUTE_TIME_SEC) {
