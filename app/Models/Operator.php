...
class Operator extends \CyberX\Models\BaseSql {
    ...
    public function insertRecord(?string $password, string $email, string $timezone): int {
        $params = [
            ':password' => $password ? \CyberX\Utils\Access::hashPassword($password) : $password,
            ':email'    => $email,
            ':timezone' => $timezone,
            ':active'   => 1,
        ];
    ...
    public function updatePassword(string $password, int $operatorId): void {
        $params = [
            ':password'     => \CyberX\Utils\Access::hashPassword($password),
    ...
    public function verifyPassword(string $password, int $operatorId): bool {
        ...
        return \CyberX\Utils\Access::verifyPassword($password, $operatorPassword);
    }
