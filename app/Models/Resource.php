...
class Resource extends \CyberX\Models\BaseSql implements \CyberX\Interfaces\ApiKeyAccessAuthorizationInterface {
    ...
    public function getTimeFrameTotal(array $ids, string $startDate, string $endDate, int $apiKey): array {
        ...
        $params[':field_edit'] = \CyberX\Utils\Constants::get()->FIELD_EDIT_EVENT_TYPE_ID;
        $query = (
            "SELECT
                event.url   AS id,
                COUNT(*)    AS cnt
    ...
    public function updateAllTotals(int $apiKey): int {
        $params = [
            ':key'          => $apiKey,
            ':field_edit'   => \CyberX\Utils\Constants::get()->FIELD_EDIT_EVENT_TYPE_ID,
        ];
