...
class Logbook extends \CyberX\Models\BaseSql {
    ...
    public function getLastSucceededEvent(int $apiKey): array {
        $params = [
            ':api_key'          => $apiKey,
            ':endpoint'         => '/sensor/',
            ':success'          => \CyberX\Utils\Constants::get()->LOGBOOK_ERROR_TYPE_SUCCESS,
            ':validation_error' => \CyberX\Utils\Constants::get()->LOGBOOK_ERROR_TYPE_VALIDATION_ERROR,
        ];
    ...
    public function rotateRequests(?int $apiKey): int {
        $params = [
            ':key'      => $apiKey,
            ':limit'    => \CyberX\Utils\Variables::getLogbookLimit(),
        ];
