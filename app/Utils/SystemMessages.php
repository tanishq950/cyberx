...
class SystemMessages {
    public static function get(int $apiKey): array {
        $messages = \CyberX\Utils\Routes::callExtra('SYSTEM_MESSAGES') ?? [];

        // get last event timestamp from event_account.lastseen to avoid long reindexing on login
        $lastLogbook = (new \CyberX\Models\Logbook())->getLastSucceededEvent($apiKey);
        ...
        for ($i = 0; $i < $iters; ++$i) {
            $message = $messages[$i];
            if ($message !== null) {
                if ($message['id'] !== \CyberX\Utils\ErrorCodes::CUSTOM_ERROR_FROM_DSHB_MESSAGES) {
                    $code = sprintf('error_%s', $message['id']);
                    $text = \Base::instance()->get($code);

                    $time = gmdate('Y-m-d H:i:s');
                    \CyberX\Utils\Timezones::localizeForActiveOperator($time);
    ...
    private static function getNoEventsMessage(array $lastLogbook): ?array {
        ...
        $inInterval = \CyberX\Utils\DateRange::inIntervalTillNow($lastEventTime, $interval);

        if (!$takeFromCache || !$inInterval) {
            if (!count($lastLogbook)) {
                return ['id' => \CyberX\Utils\ErrorCodes::THERE_ARE_NO_EVENTS_YET];
            }
        ...
        if (!$inInterval) {
            return ['id' => \CyberX\Utils\ErrorCodes::THERE_ARE_NO_EVENTS_LAST_DAY];
        }
    ...
    private static function getOveruseMessage(int $apiKey): ?array {
        ...
        if ($key->lastCallReached === false) {
            return ['id' => \CyberX\Utils\ErrorCodes::ENRICHMENT_API_KEY_OVERUSE];
        }
    ...
    private static function getInactiveCronMessage(array $lastLogbook, int $apiKey): ?array {
        $cursorModel = new \CyberX\Models\Cursor();

        if ($cursorModel->getCursor() === 0 && count($lastLogbook)) {
            return ['id' => \CyberX\Utils\ErrorCodes::CRON_JOB_MAY_BE_OFF];
        }
    ...
    private static function canTakeLastEventTimeFromCache(\CyberX\Entities\Operator $operator): bool {
        $interval = \Base::instance()->get('LAST_EVENT_CACHE_TIME');

        return !!\CyberX\Utils\DateRange::inIntervalTillNow($operator->lastEventTime, $interval);
    }

    // TODO: get message by api key?
    private static function getCustomErrorMessage(): ?array {
        $message = null;
        $model = new \CyberX\Models\Message();

        $data = $model->getLastMessage();

        if ($data) {
            $message = [
                'id'            => \CyberX\Utils\ErrorCodes::CUSTOM_ERROR_FROM_DSHB_MESSAGES,
                'text'          => $data['text'],
