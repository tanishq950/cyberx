...
class Cron {
    ...
    public static function sendBlacklistReportPostRequest(array $hashes, string $enrichmentKey): string {
        ...
        $response = \CyberX\Utils\Network::sendApiRequest($postFields, '/global_alert_report', 'POST', $enrichmentKey);

        return $response->error() ?? '';
    }

    public static function checkTimezone(string $timezone): bool {
        $hour = (new \DateTime('now', \CyberX\Utils\Timezones::getTimezone($timezone)))->format('H');
        $hour = \CyberX\Utils\Conversion::intValCheckEmpty($hour, 0);
    ...
    public static function sendUnreviewedItemsReminderEmail(string $name, string $email, int $reviewCount): bool {
        ...
        \CyberX\Utils\Mailer::send($name, $email, $subject, $message);
