...
class Navigation extends \CyberX\Controllers\Admin\Base\Navigation {
    ...
    public function manageUser(): array {
        $accountId  = \CyberX\Utils\Conversion::getIntRequestParam('userId');
        $cmd        = \CyberX\Utils\Conversion::getStringRequestParam('type');
        ...
        switch ($cmd) {
            case 'add':
                ...
                $successCode = \CyberX\Utils\ErrorCodes::USER_ADDED_TO_WATCHLIST;
                break;
            ...
            case 'fraud':
                ...
                $successCode = \CyberX\Utils\ErrorCodes::USER_FRAUD_FLAG_SET;
                break;

            case 'legit':
                $this->controller->addToBlacklistQueue($accountId, false, false, true, $this->apiKey);  // recalculate
                $successCode = \CyberX\Utils\ErrorCodes::USER_FRAUD_FLAG_UNSET;
                break;

            case 'reviewed':
                $this->controller->setReviewedFlag($accountId, true, $this->apiKey);
                $successCode = \CyberX\Utils\ErrorCodes::USER_REVIEWED_FLAG_SET;
                break;
        }
