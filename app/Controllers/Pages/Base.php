...
abstract class Base {
    ...
    public function __construct() {
        ...
        \CyberX\Utils\Routes::callExtra('PAGE_BASE');
    }
    ...
    public function getInternalPageTitleWithPostfix(string $title): string {
        $title = $title ? $title : \CyberX\Utils\Constants::get()->UNAUTHORIZED_USERID;
        $safeTitle = htmlspecialchars($title, ENT_QUOTES, 'UTF-8');
        $title = sprintf('%s %s', $safeTitle, \CyberX\Utils\Constants::get()->PAGE_TITLE_POSTFIX);
    ...
    public function applyPageParams(array $params): array {
        ...
        if ($currentOperator) {
            ...
            $params['NUMBER_OF_NOT_REVIEWED_USERS'] = \CyberX\Utils\Conversion::formatKiloValue($cnt);

            $cnt = $currentOperator->blacklistUsersCnt ?? 0;
            $params['NUMBER_OF_BLACKLIST_USERS'] = \CyberX\Utils\Conversion::formatKiloValue($cnt);

            $controller = new \CyberX\Controllers\Admin\Home\Data();
            $params += $controller->getCurrentTime($currentOperator);
        }

        $params['ALLOW_EMAIL_PHONE'] = \CyberX\Utils\Variables::getEmailPhoneAllowed();

        $page = $this->page;
        \CyberX\Utils\DictManager::load($page);
        ...
        $params = \CyberX\Utils\Routes::callExtra('APPLY_PAGE_PARAMS', $params, $page) ?? $params;
