...
class A06 extends \CyberX\Assets\Rule {
    ...
    protected function prepareParams(array $params): array {
        ...
        $pwdChange = \CyberX\Utils\Constants::get()->ACCOUNT_PASSWORD_CHANGE_EVENT_TYPE_ID;

        if (count(array_unique($params['eip_country_id'])) > 1) {
            foreach ($params['event_type'] as $idx => $event) {
                if ($event === $pwdChange) {
                    if (\CyberX\Utils\Rules::countryIsNewByIpId($params, $params['event_ip'][$idx])) {
